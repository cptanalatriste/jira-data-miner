...
public class ExtendedIssue extends BaseCsvRecord {
    ...
    public static final int MAXIMUM_RELEASES_FOR_SEVERE = 0;
    public static final int MAXIMUM_RELEASES_FOR_DEFAULT = 1;
     ...
     *
     * @param issue Original issue, with all information loaded,
     */
    ...
    private boolean isFixDelayed(int maximumReleases) {
        return this.getReleasesToBeFixed() != null
                && this.getReleasesToBeFixed() > maximumReleases;
    }

    private boolean isIssueIgnored() {
        String statusId = this.getIssue().getStatus().getId();
        return Status.OPEN.equals(statusId) || Status.REOPEN.equals(statusId);
    }

    private boolean isFixRejected() {
        return this.getIssue().getResolution() != null && !this.isAcceptedByDevTeam();
    }

    /**
     * Identifies an inflated report as true default report. The calculation is made based on the periods to be fixed.
     *
     * @return True if it is an inflated reports that should be a default. False otherwise.
     */
     ...
     * sorts it.
     *
     * @param projectVersions List of project versions.
     */
    ...
    public void configureCsvRecord() {
        ...
        this.addDataItem(JiraCsvConfiguration.IS_REJECTED, this.isFixRejected());
        this.addDataItem(JiraCsvConfiguration.IS_IGNORED, this.isIssueIgnored());
